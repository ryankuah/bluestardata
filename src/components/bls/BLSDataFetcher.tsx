...
export async function fetchBLSData(
  ...
  const labourData = (await labourResponse.json()) as [
    string,
    string,
    string,
    string,
  ...
  const topIndustries: IndustryData[] = Object.values(
    labourData
      ...
      .filter(
        (entry): entry is IndustryData =>
          entry !== null && entry.naicsCode !== "00",
      )
      .reduce<Record<string, IndustryData>>((acc, entry) => {
        ...
        if (
          !existingEntry ||
          Number(entry.naicsCode) < Number(existingEntry.naicsCode)
        ) {
  ...
  const industryMap = Object.fromEntries(
    topIndustries.map(({ naicsCode, industry }) => [naicsCode, industry]),
  );
  industryMap["10"] = "Total, all industries";

  const industryCodes = [
    "10",
    ...topIndustries.map((industry) => industry.naicsCode),
  ];
